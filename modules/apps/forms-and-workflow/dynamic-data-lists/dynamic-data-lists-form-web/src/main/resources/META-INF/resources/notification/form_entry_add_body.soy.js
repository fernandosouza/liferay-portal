...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList219 = opt_data.fields;
  var fieldListLen219 = fieldList219.length;
  for (var fieldIndex219 = 0; fieldIndex219 < fieldListLen219; fieldIndex219++) {
    var fieldData219 = fieldList219[fieldIndex219];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData219}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  var output = '';
  var pageList253 = opt_data.pages;
  var pageListLen253 = pageList253.length;
  for (var pageIndex253 = 0; pageIndex253 < pageListLen253; pageIndex253++) {
    var pageData253 = pageList253[pageIndex253];
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData253.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData253.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
